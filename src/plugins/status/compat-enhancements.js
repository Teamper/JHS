...
import { siteContext, escapeHtml } from "../../core/constants.js";
...
export class CompatibilityEnhancementsPlugin extends BasePlugin {
    ...
    async addRemoveRecord() {
        ...
        button.on("click", ((/** @type {Event} */ event) => utils.q(event, `确定移除 ${escapeHtml(carNum)} 的鉴定记录？`, (async () => { await this.getRuntimeService("state").remove(carNum); button.remove(); this.getOptionalDependency("ListPagePlugin")?.showCarNumBox?.(carNum); show.ok("鉴定记录已移除"); }))));
    }
