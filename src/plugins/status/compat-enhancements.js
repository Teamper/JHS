...
export class CompatibilityEnhancementsPlugin extends BasePlugin {
    ...
    async addRemoveRecord() {
        ...
        button.on("click", (event => utils.q(event, `确定移除 ${carNum} 的鉴定记录？`, (async () => { await stateService.remove(carNum); button.remove(); this.getDependency("ListPagePlugin")?.showCarNumBox?.(carNum); show.ok("鉴定记录已移除"); }))));
    }
