...
export default defineFeature({
    ...
    activate: (/** @type {any} */ deps, /** @type {any} */ runtime) => {
        const translationController = runtime.enabledContributions.includes("external-bridge.translation") && ["detail", "list"].includes(runtime.route)
            ? new ExternalBridgeTranslationController({ document: globalThis.document, window: globalThis.window, route: runtime.route, settings: deps[SERVICE.settings], translation: deps[SERVICE.translation], features: deps[REGISTRY.feature], styles: deps[PORT.style], ui: deps[SERVICE.ui], scope: runtime.scope })
            : null;
        ...
        const oneOneFiveController = runtime.enabledContributions.includes("external-bridge.115-match") && supportedHost
            ? new OneOneFiveMatchController({ document: globalThis.document, window: globalThis.window, route: runtime.route, hostAdapter: deps[PORT.host], offline: deps[SERVICE.offline], dialog: deps[SERVICE.dialog], settings: deps[SERVICE.settings], eventBus: deps[SERVICE.eventBus], ui: deps[SERVICE.ui], scope: runtime.scope })
            : null;
        const oneTwoThreeController = runtime.enabledContributions.includes("external-bridge.123pan")
            ? new OneTwoThreeAuthController({ document: globalThis.document, window: globalThis.window, storage: deps[SERVICE.storage], ui: deps[SERVICE.ui], scope: runtime.scope })
            : null;
