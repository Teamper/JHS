...
export default defineFeature({
    ...
    activate: (/** @type {any} */ deps, /** @type {any} */ runtime) => {
        ...
        const blacklistController = runtime.enabledContributions.includes("library.blacklist")
            ...
            : null;
        const controller = new LibraryController({ historyController, blacklistController, keywordFilterEnabled: runtime.enabledContributions.includes("library.keyword-filter"), stateImportEnabled: deps[PORT.host]?.site === "javdb" && runtime.enabledContributions.includes("library.state-actions"), favoriteActressesEnabled: deps[PORT.host]?.site === "javdb" && runtime.enabledContributions.includes("library.favorite-actresses"), hostAdapter: deps[PORT.host], storage: deps[SERVICE.storage], settings: deps[SERVICE.settings], eventBus: deps[SERVICE.eventBus], storageMutation: deps[SERVICE.storageMutation], state: deps[SERVICE.state], http: deps[SERVICE.http], route: runtime.route, scope: runtime.scope });
