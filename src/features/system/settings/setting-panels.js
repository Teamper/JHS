...
export async function renderPluginMgmtPanel(diagnostics, /** @type {any} */ settings = null) {
    ...
    const featureCatalog = new Map((diagnosticSnapshot.featureCatalog || []).map((/** @type {{id: string, featureId: string, disableable: boolean}} */ item) => [item.id, item]));
    const pluginEntries = Object.keys(pluginMeta).filter((name) => featureCatalog.has(disabledIdForPlugin(name))).filter((name, index, values) => values.findIndex((candidate) => disabledIdForPlugin(candidate) === disabledIdForPlugin(name)) === index);
    const allNames = pluginEntries;
    ...
    for (const [catKey, cat] of Object.entries(categories)) {
        ...
        for (const pName of visiblePlugins) {
            const isCore = featureCatalog.get(disabledIdForPlugin(pName))?.disableable === false;
    ...
    $(".pm-toggle").off("change").on("change", async (/** @type {Event} */ e) => {
        ...
        const renderState = (/** @type {string[]} */ disabledList) => {
            const all = allNames, currentDisabled = effectiveDisabledNames(disabledList);
    ...
    const timings = Object.entries(diagnosticSnapshot.startupTimings || {}).map(([name, elapsed]) => ({ name, elapsed: Number(elapsed), status: diagnosticSnapshot.featureStates?.[name]?.state || "active", startupMode: "feature" }));
    ...
    const errorLog = diagnosticSnapshot.errors.filter((/** @type {any} */ error) => error.source === "feature-runtime" || error.feature);
    if (errorLog.length) {
