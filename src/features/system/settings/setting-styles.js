...
/** @param {unknown} [enableVerticalModel] */
export async function applyImageMode(enableVerticalModel) {
 ...
 * @param {{ listFeatureApi?: any, hostAdapter?: any }} [options]
 */
export async function applyLayoutFromSettings(snapshot = {}, { listFeatureApi = null, hostAdapter = null } = {}) {
    const vertical = snapshot.enableVerticalModel === undefined ? C : snapshot.enableVerticalModel;
    await applyImageMode(vertical);
    ...
    if (l && listFeatureApi?.logImageHeightsByRow) {
        await listFeatureApi.logImageHeightsByRow({ vertical, columns });
    }
    if (hostAdapter) {
