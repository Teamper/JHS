...
export class SettingsCoreController {
    /** @param {{host?: any, diagnostics?: any, profile?: any, webdav?: any, dialog?: any, storage?: any, legacyStorage?: any, settings?: any, http?: any, offline?: any, magnet?: any, movie?: any, state?: any, translation?: any, eventBus?: any, settingsRegistry?: any, features?: any, screenshot?: any, ui?: any, styles?: any, scope?: any}} [options] */
    constructor(options = {}) {
        this.runtimeServices = Object.freeze({
            host: options.host ?? null, diagnostics: options.diagnostics ?? null, profile: options.profile ?? null,
            webdav: options.webdav ?? null, dialog: options.dialog ?? null, storage: options.storage ?? null,
            legacyStorage: options.legacyStorage ?? null, settings: options.settings ?? null, http: options.http ?? null,
            offline: options.offline ?? null, magnet: options.magnet ?? null, movie: options.movie ?? null,
            state: options.state ?? null, translation: options.translation ?? null, eventBus: options.eventBus ?? null,
            settingsRegistry: options.settingsRegistry ?? null, features: options.features ?? null, ui: options.ui ?? null,
            styles: options.styles ?? null, scope: options.scope ?? null,
        });
        this.folderName = "JHS-数据备份";
        ...
        this._settingScope = null;
        this._settingNavResizeCleanup = null;
        this._desktopNavGeneration = 0;
        this._settingsDialogGeneration = 0;
        ...
        /** @type {any} */
        this.resourceState = { custom: [], tags: [], filters: [], builtIn: [], screenshot: { mode: "auto", providers: [] } };
        /** @type {any} */
        this.resourceCloudState = {};
        this.started = false;
        /** @type {any[]} */
        this.cacheItems = [ {
            key: "jhs_dmm_video",
    ...
    getJQuery() { return this.getRuntimeService("ui")?.getJQuery?.() ?? /** @type {any} */ (globalThis).$; }
    getLegacyStorage() { return this.getRuntimeService("legacyStorage") ?? /** @type {any} */ (globalThis).storageManager; }
    async getLifecycleScope() {
        const scope = this.getRuntimeService("scope");
        return typeof scope === "function" ? await scope() : scope;
    }
    getSelector(/** @type {string} */ site) { return this.getRuntimeService("host")?.getListSelectors?.() ?? getDefaultListSelectors(site); }
    start() {
        if (this.started) return Promise.resolve();
        this.started = true;
        return Promise.resolve().then(() => this.handle()).catch((error) => {
            this.started = false;
            throw error;
        });
    }
    getApi() {
        return Object.freeze({
            hasSettings: true,
            openSettingDialog: (/** @type {any} */ ...args) => this.openSettingDialog(...args),
            openQuickSetting: () => this.openQuickSetting(),
            mountDesktopSettingNav: () => this.mountDesktopSettingNav(),
            unmountDesktopSettingNav: () => this.unmountDesktopSettingNav(),
        });
    }
    getName() {
    ...
    /** Resolve list-owned image capabilities without coupling settings to BusImgPlugin. */
    async getListFeatureApi() {
        ...
        try {
            ...
            clog.warn("List Feature API 不可用，跳过 JavBus 图片高度修正", error);
            this.listFeatureApi = null;
        }
    ...
    async getFormDependencies() {
        ...
        const list = await this.getListFeatureApi();
        return Object.freeze({
            otherSite: null,
            actressInfo: identity?.hasActressInfo ? identity : null, screenshot: this.getRuntimeService("screenshot"),
            newVideo: discovery?.hasNewVideo ? discovery : null, blacklist,
            listFeatureApi: list, host: this.getRuntimeService("host"), movie: this.getRuntimeService("movie"), settings: this.getRuntimeService("settings"),
    ...
    async initCss() {
        ...
        const listFeatureApi = await this.getListFeatureApi();
        applyLayoutFromSettings(e, { listFeatureApi, hostAdapter: this.getRuntimeService("host") }).catch((error => clog.error("[JHS] applyLayoutFromSettings failed:", error)));
    ...
    async handle() {
        ...
        await this.getLegacyStorage()?.getSetting?.("enableClog", _) === _ && clog.show();
        ...
        const onSettingsChanged = (/** @type {any} */ event) => {
            ...
            if (names.some((name) => [ "mobileMode", "enableVerticalModel", "containerColumns", "containerWidth" ].includes(name))) {
                void applyLayoutFromSettings(liveSettings.snapshot(), { listFeatureApi: this.listFeatureApi, hostAdapter: this.getRuntimeService("host") }).catch((/** @type {unknown} */ error) => clog.error("布局设置应用失败", error));
            }
        };
    ...
    async openSettingDialog(e = "backup-panel", t) {
        ...
        this.getRuntimeService("dialog").open({
            ...
            success: async (e, n) => {
                ...
                this.renderTaskStatuses(layerRoot), this.taskStatusUnsubscribe?.(), this.taskStatusUnsubscribe = this.getRuntimeService("eventBus")?.on?.("task-status-changed", (() => this.renderTaskStatuses(layerRoot)));
                ...
                void Promise.resolve(this.getRuntimeService("features")?.getFeatureApi?.("list"))
                    .then((/** @type {any} */ listFeature) => listFeature?.enableSvgBtn?.())
                    .catch((/** @type {unknown} */ error) => clog.warn("列表卡片操作刷新失败", error));
            }
        });
    }
    ...
    bindClick(layerRoot = null) {
        const settingPlugin = this, legacyStorage = this.getLegacyStorage(), webdav = this.getRuntimeService("webdav"), dialog = this.getRuntimeService("dialog"), diagnostics = this.getRuntimeService("diagnostics"), storage = this.getRuntimeService("storage"), translation = this.getRuntimeService("translation"), previewDiff = (diff, imported, restored = null) => showDiffPreview(diff, imported, restored, dialog);
        ...
        root.find("#createSnapshotBtn").on("click", (async () => {
            ...
            try {
                await legacyStorage.createSnapshot("手动快照", "manual"), show.ok("快照创建成功"), renderSnapshotPanel();
            } catch (error) {
        ...
        root.find(".clean-btn").on("click", (async e => {
            ...
            key === legacyStorage.third_party_cache_key ? await legacyStorage.clearThirdPartyCache() : "_circuitBreaker" === key ? diagnostics.resetAllCircuitBreakers() : "_domainStats" === key ? diagnostics.clearDomainStats() : "jhs_translate" === key ? await translation.clearCache() : storage.removeLocal(key);
        ...
        root.find("#clean-all").on("click", (async () => {
            ...
            root.find("#cache-data-display").addClass("jhs-is-hidden"), storage.removeLocal("jhs_other_site_dmm"), await legacyStorage.clearThirdPartyCache();
        }));
        root.find(".view-btn").on("click", (async e => {
            const key = $(e.currentTarget).data("key");
            let raw;
            if (key === legacyStorage.third_party_cache_key) raw = JSON.stringify(await legacyStorage.getThirdPartyCache());
    ...
    async confirmCarNumbers(event, root = $(document)) {
        ...
        const pending = this.pendingCarImport, legacyStorage = this.getLegacyStorage();
        utils.q(event, `确认导入 ${pending.values.length} 条记录？`, (async () => {
            const existing = new Map((await legacyStorage.getCarList()).map((item => [normalizeCarNum(item.carNum), item]))), summary = { added: 0, updated: 0, failed: 0 }, flag = legacyActionToFlag(pending.actionType);
    ...
    dispose() {
        ...
        this._settingsFocusCleanup?.();
        this._settingsFocusCleanup = null;
        this.unmountDesktopSettingNav();
        this.getJQuery()("#jhs-quick-setting-backdrop, #jhs-quick-setting-sheet").remove();
    }
}

/** Compatibility export for the retained disabled-plugin ID. */
export const SettingPlugin = SettingsCoreController;
