...
import { A, B, D, P, _, c, g, h, l, o, r } from "../../core/constants.js";
...
export class ListActionsController {
    /** @param {{hostAdapter?: any, settings?: any, features?: any, storage?: any, ui?: any, scope?: any, document?: Document, window?: Window}} [options] */
    constructor(options = {}) {
    ...
    getRuntimeService(name) {
        return this.runtimeServices[name] ?? ({ host: this.hostAdapter, settings: this.settings, features: this.features, storage: this.storage, ui: this.ui, scope: this.scope }[name] ?? null);
    }
    ...
    getStorage() { return this.storage ?? /** @type {any} */ (globalThis).storageManager ?? {}; }
    ...
    getHref() { return this.hostAdapter?.location?.href ?? this.window?.location?.href ?? o; }

    /** Read the actor page identity needed by blacklist batch actions. */
    getActressPageInfo() {
        const $ = this.getJQuery(), href = this.getHref();
        if (!href.includes("/actors/") && !href.includes("/star/")) throw new Error("接口调用错误, 非演员详情页");
        /** @type {string[]} */ const names = [];
        const nameSelector = this.isJavDb() ? ".actor-section-name" : ".avatar-box .photo-info .pb10";
        $(nameSelector).first().text().trim().split(",").filter(Boolean).forEach((/** @type {string} */ name) => names.push(name.trim()));
        $(".section-meta:not(:contains('影片'))").first().text().trim().split(",").filter(Boolean).forEach((/** @type {string} */ name) => names.push(name.trim()));
        const role = $(".section-meta:contains('男優')").length > 0 ? B : P;
        const movieType = names.some((name) => name.includes("無碼")) || href.includes("uncensored") ? A : D;
        ...
        if (this.isJavDb()) {
            ...
            const parts = url.pathname.split("/"), starIndex = parts.indexOf("star");
            starId = starIndex >= 0 ? parts[starIndex + 1] ?? null : null;
            blacklistUrl = starId ? `${url.origin}/star/${starId}` : null;
        }
    ...
    async getListFeatureApi() {
        ...
        try {
            ...
            this.getClog().warn?.("列表 Feature API 不可用", error);
            this.listFeatureApi = null;
        }
    ...
    async getLibraryFeatureApi() {
        ...
        try {
            ...
            this.getClog().warn?.("Library Feature API 不可用", error);
    ...
    async getDiscoveryFeatureApi() {
        ...
        try {
            ...
            this.getClog().warn?.("Discovery Feature API 不可用", error);
            this.discoveryFeatureApi = null;
        }
    ...
    async handle(options = {}) {
        ...
        const pageWindow = this.window, $ = this.getJQuery();
        if (!pageWindow.isListPage || isHitShowPage(pageWindow.location) || pageWindow.location.search.includes("handleTop=1")) return;
        const runtimeScope = this.getRuntimeService("scope"), scope = this.lifecycleScope ?? options.scope ?? (typeof runtimeScope === "function" ? await runtimeScope() : runtimeScope);
        if (!scope) return;
        this.scope = scope;
        ...
        const onSettingsChanged = (/** @type {any} */ event) => {
            ...
            void this.syncSortUi().catch((/** @type {unknown} */ error) => this.getClog().error?.("排序状态同步失败", error));
        };
    ...
    async mountOwnedRankingControls(target = null, discoveryFeatureApi = null) {
        const $ = this.getJQuery(), heading = target?.length ? target : $(".jhs-hitshow-heading");
        if (!heading.length || $("#waitCheckBtn").length) return;
        const runtimeScope = this.getRuntimeService("scope"), scope = this.lifecycleScope ?? (typeof runtimeScope === "function" ? await runtimeScope() : runtimeScope);
    ...
    async syncSortUi() {
        const settings = this.getRuntimeService("settings"), $ = this.getJQuery();
    ...
    async createMenuBtn(scope, target = null, libraryFeatureApi = null, discoveryFeatureApi = null) {
        ...
        const $ = this.getJQuery(), storage = this.getStorage(), utils = this.getUtils();
        ...
        if (r) {
            ...
            if (e) {
                ...
                if (!t) return;
                const n = "no-" + t, a = await storage.getBlacklist();
        ...
        if (l) {
            ...
            if (e) {
                const e = await storage.getBlacklist(), a = this.getActressPageInfo();
        ...
        newVideoLabel.length && newVideoLabel.empty().append(this.document.createTextNode("新作品 ("), newVideoCount, this.document.createTextNode(")"));
    }
    ...
    bindEvent(libraryFeatureApi = null) {
        const $ = this.getJQuery(), show = this.getShow(), loading = this.getLoading(), clog = this.getClog();
    ...
    bindSortMenu() {
        const $ = this.getJQuery(), clog = this.getClog(), show = this.getShow();
        ...
        $(this.document).off("click.jhsSortMenu").on("click.jhsSortMenu", ((/** @type {any} */ event) => {
    ...
    isHitShowPage() {
        return isHitShowPage(this.window.location);
    }
    /** 自渲染榜单页（热播/Top250）：排序为页内状态，初始固定“默认”，与全局 sortMethod 解耦。 */
    isOwnedRankingPage() {
        return this.isHitShowPage() || this.window.location.search.includes("handleTop=1");
    }
    ...
    isFc2ListPage() {
        return this.isJavDb() && "/advanced_search" === this.window.location.pathname && "3" === new URLSearchParams(this.window.location.search).get("type");
    }
    supportsSorting() {
        if (this.supportsLiveSorting()) return true;
        if (this.isJavDb() && (this.getHref().includes("handle") || this.getHref().includes("advanced_search"))) return false;
        return true;
    }
    supportsLiveSorting() {
    ...
    buildBatchScope() {
        const isActorPage = this.isJavDb() ? this.getHref().includes("/actors/") : this.getHref().includes("/star/");
    ...
    async openWaitCheck() {
        const $ = this.getJQuery(), storage = this.getStorage(), show = this.getShow();
        const listFeature = await this.getListFeatureApi(), e = listFeature?.getListSelectors?.();
        const t = await storage.getSetting("waitCheckCount", 5);
        let a = 0;
...
export const ListPageButtonPlugin = ListActionsController;
