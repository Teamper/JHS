...
export class DetailExternalSitesController {
    /** @param {{hostAdapter?: any, movie?: any, storage?: any, settings?: any, ui?: any, styles?: any, scope?: any, document?: Document, window?: Window}} [options] */
    constructor(options = {}) {
    ...
    /** @param {string} name */
    getRuntimeService(name) { return this.runtimeServices[name] ?? ({ host: this.hostAdapter, movie: this.movie, storage: this.storage, settings: this.settings, ui: this.ui, scope: this.scope }[name] ?? null); }
    start() {
        this.scope?.assertActive?.();
        if (this.styleRelease || this.disposed) return;
        const css = this.initCss().replace(/^\s*<style(?:\s[^>]*)?>/i, "").replace(/<\/style>\s*$/i, "");
        this.styleRelease = this.styles?.register?.("jhs-detail-external-sites", css) ?? null;
    }
    ...
    /** ON：在当前详情页挂载外部站点面板。 */
    async mount() {
        if (!this.hostAdapter?.site && !(/** @type {any} */ (globalThis)).isDetailPage) return;
    ...
    unmount() {
        ...
        this._mountGenerations.set(this.document, (this._mountGenerations.get(this.document) || 0) + 1);
        $(this.document).find("[data-jhs-other-site-box],[data-jhs-other-site-settings]").remove();
    }
    ...
    async loadOtherSite(e, t, n = {}) {
        ...
        e = normalizeCarNum(e) || this.hostAdapter?.readMovieRef?.()?.carNum || null;
        ...
        view.root.find('[data-jhs-role="site-settings"],#settingSiteBtn').off("click.jhsOtherSite").on("click.jhsOtherSite", (() => {
            ...
            try {
                if (event.currentTarget.checked) {
                    ...
                    const carNum = view.carNum || this.hostAdapter?.readMovieRef?.()?.carNum || null, site = view.configs.find((item => item.id === siteId));
...
/** Compatibility export for retained disabled-plugin IDs and older tests. */
export const OtherSitePlugin = DetailExternalSitesController;
