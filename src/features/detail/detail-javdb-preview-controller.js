...
export class DetailJavDbPreviewController {
    /** @param {{hostAdapter?: any, settings?: any, storage?: any, movie?: any, ui?: any, styles?: any, scope?: any, document?: Document, window?: Window, detailActions?: any}} [options] */
    constructor(options = {}) {
    ...
    registerStyles() {
        ...
        this.styleRelease = this.styles.register?.("jhs-detail-javdb-preview", this.initCss()) ?? null;
    }
    /** @param {{scope?: any, detailActions?: any}} [options] */
    async handle(options = {}) {
        if (!this.window?.isDetailPage) return;
        this.detailActions = options.detailActions ?? this.detailActions;
        const settingsService = this.getRuntimeService("settings");
        if (!settingsService) return;
        if (!this.lifecycleScope) this.lifecycleScope = options.scope ?? await this.resolveScope();
        if (!this.lifecycleScope) return;
        this.scope = this.lifecycleScope;
        this.registerStyles();
        if (!this._settingsListenerBound) {
            ...
            const onSettingsChanged = (/** @type {any} */ event) => {
                ...
                void Promise.resolve(this.reconfigure()).catch((error) => this.getClog().error?.("预览重配置失败", error));
            };
    ...
    reconfigure() {
        ...
        this.mountPreview();
        if (canUseDmmPreview(settings)) void this.initDmm(this.lifecycleScope).catch((error => this.getClog().error?.("预加载 DMM 失败", error)));
    ...
    mountPreview() {
        ...
        const $ = this.getJQuery(), utils = this.getUtils(), clog = this.getClog(), windowRef = this.getWindow();
    ...
    getDmmPreview(scope = this.lifecycleScope) {
        ...
        this.dmmPreviewPromise = Promise.resolve(scope || this.resolveScope()).then((requestScope => fetchDmmPreview(this.getPageInfo().carNum, this.getRuntimeService("storage"), this.getRuntimeService("movie"), requestScope))).then((result => {
    ...
    async handleVideo() {
        ...
        const hasDetailActions = Boolean(this.detailActions);
        ...
        this.detailActions?.filterOne?.(event)))),
        $("#video-filterBtn").off("click.jhsVideo").on("click.jhsVideo", ((/** @type {MouseEvent} */ event) => this.detailActions?.filterOne?.(event))),
        $("#video-favoriteBtn").off("click.jhsVideo").on("click.jhsVideo", ((/** @type {MouseEvent} */ event) => this.detailActions?.favoriteOne?.(event)));
    }
}

/** Compatibility export for the retained disabled-plugin ID. */
export const PreviewVideoPlugin = DetailJavDbPreviewController;
