...
export class DetailExternalMagnetsController {
    /** @param {{storage?: any, http?: any, magnet?: any, ui?: any, styles?: any, scope?: any, document?: Document, window?: Window}} [options] */
    constructor(options = {}) {
    ...
    start() {
        ...
        if (this.styleRelease || this.disposed) return;
        const css = this.initCss().replace(/^\s*<style(?:\s[^>]*)?>/i, "").replace(/<\/style>\s*$/i, "");
        this.styleRelease = this.styles?.register?.("jhs-detail-external-magnets", css) ?? null;
    }
    async initializeSources() {
        ...
        const magnet = this.magnet ?? this.runtimeServices?.magnet;
        ...
        this.sourceRegistry = new MagnetSourceRegistry([{
            name: "JavDB 本站", id: "native-javdb", applicable: r, enabled: r, priority: 1, search: async (/** @type {string} */ keyword, root = this.document) => parseNativeMagnets((/** @type {any} */ (root))?.jquery ? (/** @type {any} */ (root))[0] : root, "javdb"), targetUrl: () => this.window.location.href
        }, { name: "JavBus 本站", id: "native-javbus", applicable: l, enabled: l, priority: 2, search: async (/** @type {string} */ keyword, root = this.document) => parseNativeMagnets((/** @type {any} */ (root))?.jquery ? (/** @type {any} */ (root))[0] : root, "javbus"), targetUrl: () => this.window.location.href
        }, ...externalSources
    ...
    async createMagnetHub(e, options = {}) {
        ...
        e = e.replace("FC2-", "");
        const $ = this.getJQuery(), root = options.root ? $(options.root) : $(this.document), engines = [ ...this.searchEngines ];
        const storage = this.storage, t = $('<div class="magnet-container jhs-ui"></div>'), n = $('<div class="magnet-tabs"></div>'), a = "jhs_magnetHub_selectedEngine", i = storage.getLocal(a);
    ...
    async searchEngine(e, t, n, root = this.getJQuery()(this.document)) {
        ...
        if (t.search) try {
            ...
            this.getClog().error?.(`磁力源 ${t.name} 请求失败`, error);
    ...
    async searchCustomSources(keyword) {
        const configs = JSON.parse(await this.storage.getSetting("customMagnetSources", "[]"));
        ...
        const groups = await mapLimit(enabled, 4, (async (/** @type {CustomSource} */ config) => {
            ...
            } catch (cause) { const error = /** @type {{ code?: string, message?: string, status?: number, retryable?: boolean }} */ (cause); this.getClog().error?.(`自定义磁力源 ${config.name} 失败`, new ProviderError(config.id, error.code || "HTTP_ERROR", error.message || String(cause), { cause, url, status: error.status, retryable: error.retryable })); return []; }
        }));
    ...
    async searchAllSources(sources, keyword) { const groups = await mapLimit(sources, 3, (async source => { try { return await source.search(keyword); } catch (error) { this.getClog().warn?.(`磁力源 ${source.name} 聚合失败`, error); return []; } })); return deduplicateMagnetResults(groups.flat()); }
    ...
    async requestSource(sourceId, url, options = {}) {
        const response = await (this.http ?? this.runtimeServices?.http).request({
        ...
        }, this.scope ?? await this.runtimeServices?.scope?.());
    ...
    async displayResults(e, t, n) {
        ...
        t.forEach((t => {
            ...
            await this.getUtils().copyToClipboard?.("磁力链接", t) && a(e);
...
/** Compatibility export for unit tests and the retained disabled-plugin ID map. */
export const MagnetHubPlugin = DetailExternalMagnetsController;
