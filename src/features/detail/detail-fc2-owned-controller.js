...
export class Fc2OwnedDetailCoordinator {
    ...
    constructor(options = {}) {
        ...
        this.detailActions = options.detailActions ?? null;
        this.externalMagnets = options.externalMagnets ?? null;
        this.externalSites = options.externalSites ?? null;
    ...
    async getLibraryFeatureApi() {
        ...
        try {
            ...
            this.getClog().warn?.("Library Feature API 不可用，跳过 FC2 关键词过滤", error);
    ...
    async getDiscoveryFeatureApi() {
        ...
        try {
            ...
            this.getClog().warn?.("Discovery Feature API 不可用，跳过 JavDB 登录", error);
    ...
    async getLifecycleScope() {
        ...
        return this.lifecycleScope = typeof scope === "function" ? await scope() : scope;
    }
    /** @param {any} scope */
    bindLifecycleScope(scope) {
        ...
        scope.addCleanup(() => {
            ...
            $(this.getWindow()).off("pagehide.jhsFc2Detail");
    ...
    registerStyles() {
        ...
        this.styleRelease = this.styles.register?.("jhs-detail-fc2-owned", this.initCss().replace(/^\s*<style>|<\/style>\s*$/g, "")) ?? null;
    }
    /** @param {{scope?: any, externalMagnets?: any, externalSites?: any, detailActions?: any, fc2Lookup?: any}} [options] */
    async handle(options = {}) {
        this.externalMagnets = options.externalMagnets ?? this.externalMagnets;
        this.externalSites = options.externalSites ?? this.externalSites;
        this.detailActions = options.detailActions ?? this.detailActions;
        this.fc2Lookup = options.fc2Lookup ?? this.fc2Lookup;
        this.registerStyles();
        const $ = this.getJQuery();
        ...
        const windowRef = this.getWindow(), params = new URLSearchParams(windowRef?.location?.search), requestedMovieId = params.get("movieId"), carNum = params.get("carNum"), url = params.get("url"), explicitSource = params.get("source"), host = $("section").first().empty();
        ...
        $(windowRef).off("pagehide.jhsFc2Detail").one("pagehide.jhsFc2Detail", (() => context.destroy()));
    }
    ...
    initializeWorkspace(context) {
        ...
        gallery.on(`click${context.namespace}`, ".jhs-fc2-gallery-item", ((/** @type {MouseEvent} */ event) => {
            ...
            image && this.getRuntimeService("ui").showImageViewer?.(image, "", { galleryRoot: gallery[0] });
        }));
        ...
        const detailActions = this.detailActions;
        ...
        }).catch((error => this.getClog().error?.("FC2 关键词过滤初始化失败", error)));
        this.mountFc2OtherSites(context, sitesGroup, this.externalSites);
        ...
        const settings = this.getRuntimeService("settings"), handler = (/** @type {any} */ event) => {
            ...
            if (names.includes("enableLoadOtherSite")) {
                ...
                if (settings.snapshot().enableLoadOtherSite === "no") {
                    ...
                    void this.mountFc2OtherSites(context, sitesGroup, this.externalSites);
                }
            }
        };
    ...
    loadFc2Screenshot(context) {
        ...
        void Promise.resolve().then((() => this.getLifecycleScope())).then((/** @type {any} */ scope) => renderScreenshotPanel({
            ...
            if (!context.isAlive() || generation !== context.screenshotGeneration) return;
            screenshot.empty();
            this.getClog().error?.("FC2 剧照初始化失败", error);
        });
    }
    ...
    applyFc2Translation(context) {
        ...
        Promise.resolve().then((() => this.getLifecycleScope())).then((/** @type {any} */ scope) => renderTranslatedTitle({ root: context.root, carNum: context.carNum, translation: this.getRuntimeService("translation"), scope, isActive: () => context.isAlive() && (settings.snapshot().translateTitle ?? _) === _ && generation === context.translationGeneration })).catch((error => this.getClog().error?.("FC2 标题翻译失败", error)));
    }
    /** OFF：移除 FC2 已渲染的翻译节点。 */
    /** @param {Fc2DetailContext} context */
    ...
    mountFc2OtherSites(context, sitesGroup, otherSite) {
        ...
        void Promise.resolve().then((() => otherSite.loadOtherSite(context.carNum.replace("FC2-", ""), context.carNum, { root: context.root, target: sitesGroup.find('[data-jhs-role="other-sites"]'), autoDetect: !1, isActive: () => context.isAlive() && settings.snapshot().enableLoadOtherSite !== "no" && generation === context.otherSiteGeneration }))).then((/** @type {JQueryHandle | null} */ box) => {
            ...
            this.getClog().error?.("FC2 外部站点加载失败", error);
        });
    }
    ...
    createResourceGroup(title, role) { return this.getJQuery()('<section class="jhs-fc2-resource-group"><h3 class="jhs-fc2-resource-title"></h3><div></div></section>').find("h3").text(title).end().find("div").attr("data-jhs-role", role).end(); }
    /** @param {Fc2DetailContext} context */
    ...
    async configureJavDbWantButton(context, movieIdPromise) {
        ...
        try {
            ...
            } catch (error) { this.getClog().warn?.("读取 JavDB 想看状态失败，保留手动操作", error); }
            ...
            context.isAlive() && button.prop("disabled", !1).text("JavDB 关联失败，重试").off(`click${context.namespace}`).on(`click${context.namespace}`, (() => void this.configureJavDbWantButton(context, this.resolveMovieId(context.carNum)))), this.getClog().error?.("FC2 JavDB 想看关联失败", error);
        }
    }
    ...
    async submitJavDbWant(context, movieId, button) {
        ...
        try {
            ...
            if (!context.isAlive()) return;
            button.attr({ "aria-pressed": "true", "aria-disabled": "false" }).text("已加入 JavDB 想看"), this.getShow().ok?.("已加入 JavDB 想看");
        } catch (error) {
            if (!context.isAlive()) return;
            ...
            button.attr("aria-disabled", "false").text("JavDB 想看"), this.getShow().error?.(normalizedError?.message || "加入 JavDB 想看失败"), this.getClog().error?.("加入 JavDB 想看失败", error);
        } finally {
    ...
    async fetchAndRenderNativeDetail(context) {
        try {
            ...
            context.isAlive() && renderFc2State(context.root.find('[data-jhs-role="summary-content"]'), "影片信息加载失败", (() => void this.fetchAndRenderNativeDetail(context))), this.getClog().error?.("FC2 详情加载失败", error);
        }
    }
    ...
    async fetchAndRenderNativeMagnets(context, movieId = context.movieId) {
        ...
        } catch (error) { context.isAlive() && renderFc2State(host, "站内磁力加载失败", (() => void this.fetchAndRenderNativeMagnets(context, movieId))), this.getClog().error?.("FC2 磁力加载失败", error); }
    }
    ...
    async bindNativeMagnetFilter(context, host, hasMatch) {
        const $ = this.getJQuery(), show = this.getShow(), clog = this.getClog();
    ...
    async mountPanels(context, movieIdPromise) {
        try {
            ...
            renderFc2State(context.getSlot("reviews"), "评论关联失败", retry), renderFc2State(context.getSlot("related"), "相关清单关联失败", retry), this.getClog().error?.("FC2 JavDB 关联失败", error);
        }
    }
    ...
    async resolveFc2Source(record = {}) {
        ...
        try {
            const url = new URL(/** @type {string} */ (record.url || ""), this.getWindow()?.location?.origin);
            if (this.getRuntimeService("movie").matchesProviderUrl("av123", url.href)) return "123av";
            return url.origin === this.getWindow()?.location?.origin ? "fc2" : "";
        } catch { return ""; }
    }
