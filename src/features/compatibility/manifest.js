...
export default defineFeature({
    ...
    activate: (/** @type {any} */ deps, /** @type {any} */ runtime) => {
        const controller = new CompatibilityController({ hostAdapter: deps[PORT.host], styles: deps[PORT.style], storage: deps[SERVICE.storage], state: deps[SERVICE.state], features: deps[REGISTRY.feature], route: runtime.route, scope: runtime.scope });
