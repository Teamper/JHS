...
export default defineFeature({
    ...
    activate: (/** @type {any} */ deps, /** @type {any} */ runtime) => {
        const controller = new CompatibilityController({ hostAdapter: deps[PORT.host], styles: deps[PORT.style], storage: deps[SERVICE.storage], state: deps[SERVICE.state], features: deps[REGISTRY.feature], ui: deps[SERVICE.ui], route: runtime.route, enabled: runtime.enabledContributions.includes("compatibility.enhancements"), scope: runtime.scope });
