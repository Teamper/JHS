...
describe("resource settings service", () => {
    ...
    it("persists screenshot mode and 115 settings", async () => { const { instance, values } = load(); await instance.saveScreenshotSettings({ mode: "manual", providers: [{ id: "javstore", enabled: true }] }); await instance.saveCloudSettings({ enable115Offline: true, enable115Match: false, concurrency: 4, cacheMinutes: 60 }); expect(values.get("screenshotMode")).toBe("manual"); expect(values.get("enable115Offline")).toBe(true); expect(values.get("oneOneFiveConcurrency")).toBe(4); });
