...
for (const file of files) {
    ...
    if (/^\s*\/\/\s*@ts-check\b/m.test(source)) checked.push(file);
}
const coverage = files.length === 0 ? 100 : (checked.length / files.length) * 100;
const required = Number(process.env.JHS_CHECKJS_MIN ?? 50);
if (coverage + Number.EPSILON < required) {
    throw new Error(`checkJs coverage ${coverage.toFixed(1)}% is below ${required}%`);
}
console.log(`checkJs coverage: ${checked.length}/${files.length} (${coverage.toFixed(1)}%, required ${required}%)`);
