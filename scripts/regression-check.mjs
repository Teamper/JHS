...
assertIncludes(workflow, 'tags: ["v*"]', "release workflow tag trigger");
assertIncludes(workflow, "gh release create", "release workflow main release");
assertIncludes(workflow, "Reject an existing release", "release workflow immutability guard");
assert(!workflow.includes("--clobber"), "release workflow must not overwrite an existing asset");
assert(!workflow.includes("gh release upload"), "release workflow must not update an existing release");
assert(!workflow.includes("JHS-dev.user.js"), "release workflow must not build dev artifacts");
